...
function getAds(player) {
  return {
    ...
    stitchedAds: function stitchedAds(arg) {
      if (arg !== undefined) {
        videojs__default["default"].log.warn('Using player.ads.stitchedAds() as a setter is deprecated, ' + 'it should be set as an option upon initialization of contrib-ads.'); // Keep the private property and the settings in sync. When this
    ...
    isLive: function isLive(somePlayer) {
      ...
      if (typeof somePlayer.ads.settings.contentIsLive === 'boolean') {
        ...
        return true;
      } else if (videojs__default["default"].browser.IOS_VERSION === '8' && somePlayer.duration() === 0) {
        return true;
      }
    ...
    shouldPlayContentBehindAd: function shouldPlayContentBehindAd(somePlayer) {
      ...
      if (!somePlayer) {
        ...
        return !videojs__default["default"].browser.IS_IOS && !videojs__default["default"].browser.IS_ANDROID && somePlayer.duration() === Infinity;
      }
    },
    ...
    debug: function debug() {
      if (this.settings.debug) {
        ...
        if (args.length === 1 && typeof args[0] === 'string') {
          videojs__default["default"].log('ADS: ' + args[0]);
        } else {
          videojs__default["default"].log.apply(videojs__default["default"], ['ADS:'].concat(args));
        }
      }
    }
  };
...
var proxyTcfApi = function proxyTcfApi(_) {
  if (videojs__default["default"].dom.isInFrame() && typeof window__default["default"].__tcfapi !== 'function') {
    var frame = window__default["default"];
    var cmpFrame;
    var cmpCallbacks = {};
    ...
    while (frame) {
      ...
      if (frame === window__default["default"].top) {
        break;
      }
    ...
    window__default["default"].__tcfapi = function (cmd, version, callback, arg) {
    ...
    window__default["default"].addEventListener('message', function (event) {
      var json = {};
...
var listenToTcf = function listenToTcf() {
  ...
  if (typeof window__default["default"].__tcfapi === 'function') {
    window__default["default"].__tcfapi('addEventListener', 2, function (data, success) {
      if (success) {
        tcData = data;
      }
    });
...
function adMacroReplacement(string, uriEncode, customMacros) {
  ...
  macros['{player.pageUrl}'] = videojs__default["default"].dom.isInFrame() ? document__default["default"].referrer : window__default["default"].location.href;
  ...
  macros['{document.referrer}'] = document__default["default"].referrer;
  macros['{window.location.href}'] = window__default["default"].location.href;
  ...
  string = string.replace(/{pageVariable\.([^}]+)}/g, function (match, name) {
    var value;
    var context = window__default["default"];
    ...
    if (value === null) {
      ...
      videojs__default["default"].log.warn("Page variable \"" + name + "\" not found");
      return '';
    } else if (type !== 'string' && type !== 'number' && type !== 'boolean') {
      videojs__default["default"].log.warn("Page variable \"" + name + "\" is not a supported type");
      return '';
    }
...
cueTextTracks.processAdTrack = function (player, cues, processCue, cancelAdsHandler) {
  ...
  for (var i = 0; i < cues.length; i++) {
    ...
    if (!this.isSupportedAdCue(player, cue)) {
      videojs__default["default"].log.warn('Skipping as this is not a supported ad cue.', cue);
      return;
    ...
    if (cueAlreadySeen(player, cueId)) {
      videojs__default["default"].log('Skipping ad already seen with ID ' + cueId);
      return;
...
function initCancelContentPlay(player, debug) {
  if (debug) {
    videojs__default["default"].log('Using cancelContentPlay to block content playback');
...
var videojsReference = videojs__default["default"];
/**
    ...
    play: function play(terminated, playPromise) {
      if (player.ads && player.ads._playBlocked && terminated) {
        ...
        playPromise.catch(function (e) {
          if (e.name === 'NotAllowedError' && !videojs__default["default"].browser.IS_SAFARI) {
...
var hasAdsPlugin = function hasAdsPlugin() {
  ...
  if (videojs__default["default"].getPlugin) {
    return Boolean(videojs__default["default"].getPlugin('ads'));
...
function register(contribAdsPlugin) {
  ...
  var registerPlugin = videojs__default["default"].registerPlugin || videojs__default["default"].plugin; // Register this plugin with Video.js.
  ...
  if (isMiddlewareMediatorSupported() && !videojs__default["default"].usingContribAdsMiddleware_) {
    // Register the play middleware
    videojs__default["default"].use('*', playMiddleware);
    videojs__default["default"].usingContribAdsMiddleware_ = true;
    videojs__default["default"].log.debug('Play middleware has been registered with videojs');
  }
...
var State = /*#__PURE__*/function () {
  ...
  _proto.onAdsReady = function onAdsReady() {
    videojs__default["default"].log.warn('Unexpected adsready event');
  };
  ...
  _proto.onReadyForPostroll = function onReadyForPostroll() {
    videojs__default["default"].log.warn('Unexpected readyforpostroll event');
  };
  ...
  _proto.startLinearAdMode = function startLinearAdMode() {
    videojs__default["default"].log.warn('Unexpected startLinearAdMode invocation ' + '(State via ' + this.constructor._getName() + ')');
  };

  _proto.endLinearAdMode = function endLinearAdMode() {
    videojs__default["default"].log.warn('Unexpected endLinearAdMode invocation ' + '(State via ' + this.constructor._getName() + ')');
  };

  _proto.skipLinearAdMode = function skipLinearAdMode() {
    videojs__default["default"].log.warn('Unexpected skipLinearAdMode invocation ' + '(State via ' + this.constructor._getName() + ')');
  }
  /*
...
var AdsDone = /*#__PURE__*/function (_ContentState) {
  ...
  _proto.startLinearAdMode = function startLinearAdMode() {
    videojs__default["default"].log.warn('Unexpected startLinearAdMode invocation (AdsDone)');
  };
...
function getPlayerSnapshot(player) {
  ...
  if (videojs__default["default"].browser.IS_IOS && player.ads.isLive(player)) {
...
function restorePlayerSnapshot(player, callback) {
    ...
    if (videojs__default["default"].browser.IS_IOS && player.ads.isLive(player)) {
      ...
      if (player.paused()) {
        ...
        if (playPromise && playPromise.catch) {
          playPromise.catch(function (error) {
            videojs__default["default"].log.warn('Play promise rejected in IOS snapshot resume', error);
          });
        }
      ...
      if (_playPromise && _playPromise.catch) {
        _playPromise.catch(function (error) {
          videojs__default["default"].log.warn('Play promise rejected in snapshot resume', error);
        });
      }
  ...
  var tryToResume = function tryToResume() {
    ...
    if (attempts--) {
      ...
      try {
        ...
        videojs__default["default"].log.warn('Failed to resume the content after an advertisement', e);
      }
    }
  };
  ...
  if (player.ads.videoElementRecycled()) {
    ...
    if (videojs__default["default"].browser.IS_IOS && !player.autoplay()) {
    ...
    if (!player.ended()) {
      ...
      if (playPromise && playPromise.catch) {
        playPromise.catch(function (error) {
          videojs__default["default"].log.warn('Play promise rejected in snapshot restore', error);
        });
      }
...
var Preroll = /*#__PURE__*/function (_AdState) {
  ...
  _proto.onAdsReady = function onAdsReady(player) {
    if (!player.ads.inAdBreak()) {
      ...
      videojs__default["default"].log.warn('Unexpected adsready event (Preroll)');
    }
  }
  /*
  ...
  _proto.onAdsError = function onAdsError(player) {
    ...
    videojs__default["default"].log('adserror (Preroll)'); // In the future, we may not want to do this automatically.
  ...
  _proto.startLinearAdMode = function startLinearAdMode() {
    ...
    if (this.adsReady && !player.ads.inAdBreak() && !this.isContentResuming()) {
      ...
      videojs__default["default"].log.warn('Unexpected startLinearAdMode invocation (Preroll)');
    }
  }
  /*
  ...
  _proto.skipLinearAdMode = function skipLinearAdMode() {
    ...
    if (player.ads.inAdBreak() || this.isContentResuming()) {
      videojs__default["default"].log.warn('Unexpected skipLinearAdMode invocation');
    } else {
      this.afterLoadStart(function () {
  ...
  _proto.onNoPreroll = function onNoPreroll(player) {
    if (player.ads.inAdBreak() || this.isContentResuming()) {
      videojs__default["default"].log.warn('Unexpected nopreroll event (Preroll)');
    } else {
      this.noPreroll();
    }
  };
  ...
  _proto.cleanup = function cleanup(player) {
    if (!player.ads._hasThereBeenALoadStartDuringPlayerLife) {
      videojs__default["default"].log.warn('Leaving Preroll state before loadstart event can cause issues.');
    }
...
var Postroll = /*#__PURE__*/function (_AdState) {
  ...
  _proto.startLinearAdMode = function startLinearAdMode() {
    ...
    if (!player.ads.inAdBreak() && !this.isContentResuming()) {
      ...
      videojs__default["default"].log.warn('Unexpected startLinearAdMode invocation (Postroll)');
    }
  }
  /*
   * An ad has actually started playing.
  ...
  _proto.onNoPostroll = function onNoPostroll(player) {
    if (!this.isContentResuming() && !this.inAdBreak()) {
      ...
      videojs__default["default"].log.warn('Unexpected nopostroll event (Postroll)');
    }
  };
...
var VIDEO_EVENTS = videojs__default["default"].getTech('Html5').Events; // Default settings
...
var defaults$1 = {
  ...
  allowVjsAutoplay: videojs__default["default"].options.normalizeAutoplay || false
};

var contribAdsPlugin = function contribAdsPlugin(options) {
  var player = this; // eslint-disable-line consistent-this

  var settings = videojs__default["default"].mergeOptions(defaults$1, options); // Prefix all video element events during ad playback
  ...
  player.on(['addurationchange', 'adcanplay'], function () {
    ...
    if (playPromise && playPromise.catch) {
      playPromise.catch(function (error) {
        videojs__default["default"].log.warn('Play promise rejected when playing ad', error);
      });
    }
  });
  ...
  var shouldDisableTracks = function shouldDisableTracks() {
    ...
    return !player.ads.shouldPlayContentBehindAd(player) && player.ads.inAdBreak() && player.tech_.featuresNativeTextTracks && videojs__default["default"].browser.IS_IOS && // older versions of video.js did not use an emulated textTrackList
...
contribAdsPlugin.VERSION = version$1; // Attempt to register the plugin, if we can.

register(contribAdsPlugin);

var version = "1.1.2011";
...
var VPAIDFLASHClient = require('vpaid-flash-client/bin/VPAIDFLASHClient'); // Default options for the plugin.
...
const onPlayerReady = (player, options) => {
  // console.log('test-vastWaterfall-ready-player:', player, options);
  var markers = [];
  var playedMidrolls = [];
  ...
  skipButton.innerText = "رد کردن تبلیغ";
  ...
  function prepareAds(type) {
    ...
    function playAd() {
      ...
      function loadJSAdUnit(creative) {
        ...
        function onLoad(err, adUnit) {
          ...
          function onHandShake(error, result) {
            ...
            adUnit.initAd(initialDimensions.width, initialDimensions.height, "normal", -1, {
      ...
      function loadFlashAdUnit(creative) {
        var flashVPaid = new VPAIDFLASHClient(vpaidContainer, flashWrapperLoaded, {
        ...
        function runFlashAdUnit(error, adUnit) {
          ...
          function initAd(err, result) {
            var initialDimensions = getPlayerDimensions();
            adUnit.initAd(initialDimensions.width, initialDimensions.height, "normal", mediaFile.bitRate, {
              AdParameters: creative.adParameters
            }, "", function (err) {
              player.trigger("ads-ad-started");
              player.on("resize", resizeAd);
    ...
    function endOfGroup() {
      ...
      player.trigger("ads-ad-ended"); //The pod has successfully played an ad. Let's move to the next one and reset the position
...
const vastWaterfall = function (options = {}) {
  // console.log('test-vastWaterfall:', options);
  this.ready(() => {
    // console.log('test-vastWaterfall-ready-player');
    onPlayerReady(this, videojs__default["default"].mergeOptions(defaults, options));
  });
}; // Register the plugin with video.js.


registerPlugin("vastWaterfall", vastWaterfall); // Include the version number.

vastWaterfall.VERSION = version;

module.exports = vastWaterfall;
